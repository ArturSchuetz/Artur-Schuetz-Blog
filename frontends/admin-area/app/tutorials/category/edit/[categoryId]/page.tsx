...
export default function EditTutorialCategory({
  params,
}: {
  ...
  const [category, setTutorialCategory] = useState<TutorialCategory>({
    id: 0,
    position: 0,
    name: "",
    slug: "",
      ...
      if (event.ctrlKey || event.metaKey) {
        switch (String.fromCharCode(event.which).toLowerCase()) {
          case "s":
            ...
            const updatedTutorialCategory = { ...category };
            updateTutorialCategory(updatedTutorialCategory)
              .then((updatedTutorialCategoryResponse) => {
                setTutorialCategory(updatedTutorialCategoryResponse);
              })
              .catch((error) => {
  ...
  useEffect(() => {
    getTutorialCategory(categoryId)
      .then((fetchedTutorialCategory) => {
        setTutorialCategory(fetchedTutorialCategory);
      })
      .catch((error) => {
        if (error instanceof UnauthorizedException) {
          console.error("Unauthorized!");
  ...
  const onFileUploaded = (mediaId: number) => {
    updateTutorialCategoryImage(categoryId, mediaId)
      .then((updatedTutorialCategoryResponse) => {
        setTutorialCategory(updatedTutorialCategoryResponse);
      })
      .catch((error) => {
    ...
    deleteMedia(mediaId)
      .then(() => {
        if (category && category.medias) {
          ...
          setTutorialCategory({ ...category, medias: updatedMedias });
        }
      })
      .catch((error) => {
        if (error instanceof UnauthorizedException) {
          ...
          <div className="side-app">
            <div className="main-container container-fluid">
              <div className="page-header">
                <h1 className="page-title">Edit Tutorial Category</h1>
                <div>
                  <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item">
                      <Link href={`/tutorials/category`}>Blog Categories</Link>
                    </li>
