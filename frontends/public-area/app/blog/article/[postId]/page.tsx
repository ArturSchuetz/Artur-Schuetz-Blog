...
import { BlogArticle, getBlogArticleById, getBlogArticleMetaById } from "@/app/_services/blog-article.service";
...
export async function generateMetadata({ params }: { params: { postId: number | string } }) {
  ...
  let article: BlogArticle | null = await getBlogArticleMetaById(postId);
  ...
  const canonical_site_url = '/blog/article/' + article.slug;
  ...
  return {
    ...
    openGraph: {
      ...
      section: article.category ? article.category?.name : null,
      determiner: 'auto',
      title: page_title,
      description: site_description,
...
export default async function BlogPost({ params }: { params: { postId: number } }) {
  const { postId } = params;

  let article: BlogArticle | null = await getBlogArticleById(postId);
        ...
        <ul className="d-flex justify-content-between">
          {article && article.previousArticle && (
            <li className="article-navigation-item">
              <Link
                className="article-navigation-item-link"
                href={`/blog/article/${article.previousArticle.slug}`}
                title="Previous Article"
              >
                <div className="d-flex align-items-center justify-content-center justify-content-md-start">
                  <i className="pe-7s-angle-left"></i>
                  <p className="font-primary mb-1">Previous article</p>
                </div>
                <h3 className="d-none d-md-block">{article.previousArticle.title}</h3>
              </Link>
            </li>
          )}
          {article && article.nextArticle && !article.previousArticle && (
            <li className="article-navigation-item"></li>
          )}
          {article && article.nextArticle && (
            <li className="article-navigation-item">
              <Link
                className="article-navigation-item-link"
                href={`/blog/article/${article.nextArticle.slug}`}
                title="Next Article"
              >
                <div className="d-flex aling-items-center flex-row-reverse flex-md-row justify-content-center justify-content-md-start">
                  <i className="pe-7s-angle-right"></i>
                  <p className="font-primary mb-1">Next article</p>
                </div>
                <h3 className="d-none d-md-block">{article.nextArticle.title}</h3>
              </Link>
            </li>
          )}
        </ul>
